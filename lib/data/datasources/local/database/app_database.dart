...
class PlaceImagesTable extends Table {
  @override
  String get tableName => 'place_images';
...
class SocialLinksTable extends Table {
  @override
  String get tableName => 'social_links';
...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> _createFts5Table() async {
    await customStatement('''
      CREATE VIRTUAL TABLE IF NOT EXISTS places_fts USING fts5(
        place_id UNINDEXED, name_ar, name_en,
    ...
    await customStatement('''
      CREATE TRIGGER IF NOT EXISTS places_ai AFTER INSERT ON places BEGIN
        ...
        VALUES(new.id,new.name_ar,new.name_en,new.description_ar,new.description_en,new.address_ar,new.address_en);
      END
    ''');
    await customStatement('''
  ...
  Future<void> _createIndexes() async {
    ...
    await customStatement('CREATE INDEX IF NOT EXISTS idx_pc_cat ON place_categories(category_id)');
    ...
    await customStatement('CREATE INDEX IF NOT EXISTS idx_msg_session ON chat_messages(session_id)');
  }
}
...
LazyDatabase _openConnection() {
  return LazyDatabase(() async {
    ...
    final file = File(p.join(dbFolder.path, 'localai.db'));
