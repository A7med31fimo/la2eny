  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(locale == 'ar' ? 'ترتيب حسب' : 'Sort By', style: AppTextStyles.headlineMedium),
        const SizedBox(height: 16),
        ...options.map((opt) {
          final (value, label, icon) = opt;
          final sel = current == value;
          return ListTile(
            onTap: () { ref.read(sortByNotifierProvider.notifier).setSort(value); Navigator.pop(context); },
            leading: Icon(icon, color: sel ? AppColors.primary : AppColors.textSecondary),
            ...
            tileColor: sel ? AppColors.primaryContainer : null);
        }),
      ]),
    );
  }
